import getCurrentPlaylingTrack from "@/actions/getCurrentPlaylingTrack";
...
export default async function ListenBar() {
    ...
    const currentTrack = await getCurrentPlaylingTrack();
        ...
        <>
            <div className="flex justify-between h-[72px] text-white">
                <div className="w-3/12 flex space-x-4 items-center p-[7px]">
                    <Image src={currentTrack.item.album.images[0]?.url} alt={currentTrack.item.name} className="rounded" priority={true} width={60} height={60} style={{ width: 56, height: 'auto'}}  />
                    <div className="flex flex-col justify-center">
                        <p className="text-[14px] font-medium">{currentTrack.item.name}</p>
                        <p className="text-[10.5px] font-medium text-neutral-400">{currentTrack.item.album.artists.map((artist, artistIndex) => (
