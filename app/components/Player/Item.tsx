...
export default function Item({ is_playing }: ItemProps) {
    const router = useRouter();

    useEffect(() => {
        const handleSpacebar = async (event: KeyboardEvent) => {
            if (event.code === 'Space') {
                event.preventDefault(); // Empêcher le comportement par défaut de la touche Espace (scroll)
                try {
                    if (is_playing) {
                        await pausePlayback();
                    } else {
    ...
    return (
        <form className="flex space-x-[24px] items-center h-8">
            <button>
            ...
            {is_playing ? 
                (
                    <button 
                        className="bg-[#FFFFFF] p-[8px] rounded-full"
                        formAction={async () => {
                            try {
                                await pausePlayback();
                                setTimeout(() => {
                                ...
                                console.error('Erreur lors du pause:', error);
                            }
                        }}
                    >
                        <svg data-encore-id="icon" role="img" width="16" height="16" aria-hidden="true" viewBox="0 0 16 16"><path d="M2.7 1a.7.7 0 0 0-.7.7v12.6a.7.7 0 0 0 .7.7h2.6a.7.7 0 0 0 .7-.7V1.7a.7.7 0 0 0-.7-.7H2.7zm8 0a.7.7 0 0 0-.7.7v12.6a.7.7 0 0 0 .7.7h2.6a.7.7 0 0 0 .7-.7V1.7a.7.7 0 0 0-.7-.7h-2.6z"></path></svg>
                    </button>
                ) : (
                    <button 
                        className="bg-[#FFFFFF] p-[8px] rounded-full"
                    ...
                    >
                        <svg data-encore-id="icon" role="img" width="16" height="16" aria-hidden="true" viewBox="0 0 24 24"><path d="m7.05 3.606 13.49 7.788a.7.7 0 0 1 0 1.212L7.05 20.394A.7.7 0 0 1 6 19.788V4.212a.7.7 0 0 1 1.05-.606z"></path></svg>
                    </button>
                )
            }
            {/* Skip button */}
